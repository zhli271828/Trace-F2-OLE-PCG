...
#define DPF_MSG_NUM (sizeof(struct GR128_D4)*MSG_NUM/sizeof(uint128_t))
...
void SPDZ2k_64_D4_bench_pcg(size_t n, size_t c, size_t t, struct PCG_Time *pcg_time) {
    ...
    printf("k=%zu, s=%zu\n", k, s);
    struct GR128_D4 zeta_powers[base];
    memset(zeta_powers, 0, base*sizeof(struct GR128_D4));
    struct GR128_D4 **frob_powers = xcalloc(m, sizeof(void *));
    struct GR128_D4 **power_scalar = xcalloc(m, sizeof(void *));
    for (size_t i = 0; i < m; ++i) {
        frob_powers[i] = xcalloc(m, sizeof(struct GR128_D4));
        power_scalar[i] = xcalloc(m, sizeof(struct GR128_D4));
    }

    compute_gr128_d4_zeta_powers(zeta_powers, base);
    compute_gr128_d4_Frob_map_zeta_powers(zeta_powers, frob_powers, m);
    compute_gr128_d4_power_scalar(zeta_powers, frob_powers, power_scalar, m);

    init_SPDZ2k_64_HD_bench_params(param, n, c, t, m, k, s);
    ...
    struct FFT_GR128_D4_Trace_A *fft_gr128_d4_trace_a = xcalloc(1, sizeof(struct FFT_GR128_D4_Trace_A));
    init_FFT_GR128_d4_Trace_A(param, fft_gr128_d4_trace_a);
    sample_SPDZ2k_64_d4_a_and_tensor(param, fft_gr128_d4_trace_a);
     ...
     */
    struct SPDZ2k_64_D4_b *spdz2k_64_d4_b = xcalloc(1, sizeof(struct SPDZ2k_64_D4_b));
    init_SPDZ2k_64_d4_b(param, spdz2k_64_d4_b);

    struct SPDZ2k_64_D4_Prod *spdz2k_64_d4_prod = xcalloc(1, sizeof(struct SPDZ2k_64_D4_Prod));
    init_SPDZ2k_64_d4_prod(param, spdz2k_64_d4_prod);

    printf("Benchmarking PCG evaluation\n");
    clock_t start_expand_time = clock();
    run_SPDZ2k_64_d4_b(param, spdz2k_64_d4_b, fft_gr128_d4_trace_a->fft_a, zeta_powers);
    run_SPDZ2k_64_d4_prod(param, spdz2k_64_d4_prod, fft_gr128_d4_trace_a->fft_a_tensor, power_scalar, zeta_powers);
    ...
    free_SPDZ2k_64_d4_prod(param, spdz2k_64_d4_prod);
    free_SPDZ2k_64_d4_b(param, spdz2k_64_d4_b);
    free_FFT_GR128_d4_Trace_A(param, fft_gr128_d4_trace_a);
    free(param);
    for (size_t i = 0; i < m; ++i) {
        free(frob_powers[i]);
        free(power_scalar[i]);
...
void free_SPDZ2k_64_d4_b(const struct Param *param, struct SPDZ2k_64_D4_b *spdz2k_64_d4_b) {
    free_SPDZ2k_64_d4_b_DPF_keys(param, spdz2k_64_d4_b->keys_b0);
    free_SPDZ2k_64_d4_b_DPF_keys(param, spdz2k_64_d4_b->keys_b1);
    for (size_t i = 0; i < param->m; ++i) {
        free(spdz2k_64_d4_b->b0[i]);
        free(spdz2k_64_d4_b->b1[i]);
        free(spdz2k_64_d4_b->bm0[i]);
        free(spdz2k_64_d4_b->bm1[i]);
    }
    free(spdz2k_64_d4_b->b0);
    free(spdz2k_64_d4_b->b1);
    free(spdz2k_64_d4_b->bm0);
    free(spdz2k_64_d4_b->bm1);
    
    for (size_t i = 0; i < MSG_NUM; ++i) {
        free(spdz2k_64_d4_b->polys[i]);
        free(spdz2k_64_d4_b->poly_buf[i]);
        free(spdz2k_64_d4_b->z_poly[i]);
    }
    free(spdz2k_64_d4_b->polys);
    free(spdz2k_64_d4_b->poly_buf);
    free(spdz2k_64_d4_b->z_poly);

    free(spdz2k_64_d4_b->shares);
    free(spdz2k_64_d4_b->cache);
    free(spdz2k_64_d4_b);
}
...
void free_SPDZ2k_64_d4_prod(const struct Param *param, struct SPDZ2k_64_D4_Prod *spdz2k_64_d4_prod) {
    free_SPDZ2k_64_d4_prod_DPF_keys(param, spdz2k_64_d4_prod->keys);

    for (size_t i = 0; i < MSG_NUM; ++i) {
        free(spdz2k_64_d4_prod->polys[i]);
        free(spdz2k_64_d4_prod->poly_buf[i]);
        free(spdz2k_64_d4_prod->z_poly[i]);
    }
    free(spdz2k_64_d4_prod->polys);
    free(spdz2k_64_d4_prod->poly_buf);
    free(spdz2k_64_d4_prod->z_poly);
    for (size_t i = 0; i < param->m; ++i) {
        free(spdz2k_64_d4_prod->rlt[i]);
    }
    free(spdz2k_64_d4_prod->rlt);
    free(spdz2k_64_d4_prod->shares);
    free(spdz2k_64_d4_prod->cache);
    free(spdz2k_64_d4_prod);
}
...
void run_SPDZ2k_64_d4_prod(const struct Param *param, struct SPDZ2k_64_D4_Prod *spdz2k_64_d4_prod, struct GR128_D4 **fft_a_tensor, struct GR128_D4 **power_scalar, const struct GR128_D4 *zeta_powers) {
    size_t poly_size = param->poly_size;
    struct KeysHD *keys = spdz2k_64_d4_prod->keys;

    uint128_t **rlt = spdz2k_64_d4_prod->rlt;
    
    struct GR128_D4 **polys = spdz2k_64_d4_prod->polys;
    struct GR128_D4 **poly_buf = spdz2k_64_d4_prod->poly_buf;

    struct GR128_D4 **z_poly = spdz2k_64_d4_prod->z_poly;
    uint128_t *shares = spdz2k_64_d4_prod->shares;
    uint128_t *cache = spdz2k_64_d4_prod->cache;

    // evaluate for z and K*z together
    evaluate_SPDZ2k_64_d4_prod_DPF_and_sum(param, keys, fft_a_tensor, polys, poly_buf, z_poly, shares, cache, power_scalar, zeta_powers);
    // map to trace functions
    trace_SPDZ2k_64_d4_prod_FFT_polys(param, z_poly, rlt);
}
...
void trace_SPDZ2k_64_d4_prod_FFT_polys(const struct Param *param, struct GR128_D4 **z_poly, uint128_t **rlt) {
    ...
    const size_t m = param->m;
    const uint128_t modulus64 = param->modulus64;

    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < poly_size; ++j) {
            rlt[i][j] = trace_GR128_d4(&z_poly[0][i*poly_size+j], m);
            rlt[i][j+poly_size] = trace_GR128_d4(&z_poly[1][i*poly_size+j], m);
        }
    }
}
...
void evaluate_SPDZ2k_64_d4_prod_DPF_and_sum(
    ...
    evaluate_SPDZ2k_64_d4_prod_DPF(param, keys, polys, shares, cache);
    // TODO: add the powers
    convert_SPDZ2k_64_d4_prod_to_FFT(param, polys, zeta_powers);
    multiply_SPDZ2k_64_d4_prod_FFT(param, fft_a_tensor_maps, polys, poly_buf);
    sum_SPDZ2k_64_d4_prod_FFT_polys(param, poly_buf, z_poly, power_scalar);
}
        ...
        for (size_t j = 0; j < c; ++j) {
            for (size_t k = 0; k < m; ++k) {
                ...
                for (size_t l = 0; l < m; ++l) {
                    ...
                    add_SPDZ2k_64_d4_list_scalar(&poly_buf[0][index*poly_size], &power_scalar[k][l], &z_poly[0][l*poly_size], &z_poly[0][l*poly_size], poly_size, modulus64);
                    add_SPDZ2k_64_d4_list_scalar(&poly_buf[1][index*poly_size], &power_scalar[k][l], &z_poly[1][l*poly_size], &z_poly[1][l*poly_size], poly_size, modulus64);
                }
            }
        }
    }
...
void compute_gr128_d4_zeta_powers(struct GR128_D4 *zeta_powers, const size_t m) {
    struct GR128_D4 zeta = {0};
    zeta.c0 = 0; zeta.c1=1; zeta.c2=0;
    // compute zeta_powers
    // struct GR128_D4 zeta_powers[m];
    zeta_powers[0].c0 = 1; 
    zeta_powers[0].c1 = 0;
    zeta_powers[0].c2 = 0;

    for (size_t i = 1; i<m; ++i) {
        mult_gr128_D4(&zeta, &zeta_powers[i-1], &zeta_powers[i]);
    }
}

void compute_gr128_d4_Frob_map_zeta_powers(const struct GR128_D4 *powers, struct GR128_D4 **frob_powers, const size_t m) {

    memcpy(&frob_powers[0][0], powers, sizeof(struct GR128_D4)*m);
    for (size_t i = 1; i < m; ++i) {
        for (size_t j = 0; j < m; ++j) {
            compute_Frob_gr128_d4(&frob_powers[i-1][j], &frob_powers[i][j]);
        }
    }
}
...
void compute_gr128_d4_power_scalar(const struct GR128_D4 *zeta_powers, struct GR128_D4 **frob_powers, struct GR128_D4 **power_scalar, const size_t m) {
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < m; ++j) {
            mult_gr128_D4(&frob_powers[i][j], &zeta_powers[j], &power_scalar[i][j]);
        }
    }
}
...
void add_SPDZ2k_64_d4_list_scalar(const struct GR128_D4 *a, const struct GR128_D4 *scalar, const struct GR128_D4 *b, struct GR128_D4 *t, size_t length, const size_t modulus) {
    struct GR128_D4 mult_rlt = {0};
    for (size_t i = 0; i < length; i++) {
        mult_gr128_D4(scalar, &a[i], &mult_rlt);
        add_SPDZ2k_64_d4(&mult_rlt, &b[i], &t[i], modulus);
    }
}
...
void convert_SPDZ2k_64_d4_prod_to_FFT(const struct Param *param, struct GR128_D4 **polys, const struct GR128_D4 *zeta_powers) {
    ...
    for (size_t i = 0; i < c*c*m; ++i) {
        ...
        struct GR128_D4 *poly0 = &polys[0][i*poly_size];
        struct GR128_D4 *poly1 = &polys[1][i*poly_size];
        fft_recursive_SPDZ2k_64_D4(poly0, zeta_powers, n, poly_size / base, modulus64, base);
        fft_recursive_SPDZ2k_64_D4(poly1, zeta_powers, n, poly_size / base, modulus64, base);
    }
}
...
void multiply_SPDZ2k_64_d4_prod_FFT(const struct Param *param, struct GR128_D4 **a_polys, struct GR128_D4 **b_poly, struct GR128_D4 **res_poly) {
    ...
    for (size_t i = 0; i < c*c*m; ++i) {
        mult_SPDZ2k_64_d4_list(a_polys[i], &b_poly[0][i*poly_size], &res_poly[0][i*poly_size], poly_size, modulus64);
        mult_SPDZ2k_64_d4_list(a_polys[i], &b_poly[1][i*poly_size], &res_poly[1][i*poly_size], poly_size, modulus64);
    }
}
            ...
            for (size_t w = 0; w < m; ++w) {
                for (size_t k = 0; k < t; k++) {
                    for (size_t l = 0; l < t; l++) {
                        ...
                        struct GR128_D4 *poly_block0 = &polys[0][key_index*dpf_block_size];
                        struct GR128_D4 *poly_block1 = &polys[1][key_index*dpf_block_size];

                        // DPF evaluates the value and the MAC in consecutive positions and copy_gr128_d4_block parses the DPF evaluation results
                        DPFFullDomainEvalZ(base, dpf_key, cache, shares);
                        copy_gr128_d4_block(poly_block0, poly_block1, shares, dpf_block_size);
                    }
                }
            }
        }
...
void init_SPDZ2k_64_d4_b(const struct Param *param, struct SPDZ2k_64_D4_b *spdz2k_64_d4_b) {
    ...
    sample_SPDZ2k_64_d4_b_DPF_keys(param, keys_b0);
    struct KeysHD *keys_b1 = xcalloc(1, sizeof(struct KeysHD));
    sample_SPDZ2k_64_d4_b_DPF_keys(param, keys_b1);
    // output for (Tr(zeta^j*b0), Tr(zeta^j*b1)) and each has m values for the trace
    uint128_t **b0 = xcalloc(m, sizeof(void*));
    uint128_t **b1 = xcalloc(m, sizeof(void*));
    // output for (Tr(zeta^j*K*b0), Tr(zeta^j*K*b1)) and each has m values for the trace
    uint128_t **bm0 = xcalloc(m, sizeof(void*));
    uint128_t **bm1 = xcalloc(m, sizeof(void*));
   
    for (size_t i = 0; i < m; ++i) {
        b0[i] = xcalloc(poly_size, sizeof(uint128_t));
        b1[i] = xcalloc(poly_size, sizeof(uint128_t));
        bm0[i] = xcalloc(poly_size, sizeof(uint128_t));
        bm1[i] = xcalloc(poly_size, sizeof(uint128_t));
    }

    // 2*c*poly_size for the secret of (b,K*b) and each has c parts.
    // For (b, K*b) with dimension MSG_NUM*(c*poly_size)
    struct GR128_D4 **polys = xcalloc(MSG_NUM, sizeof(void*));
    struct GR128_D4 **poly_buf = xcalloc(MSG_NUM, sizeof(void*));
    // For b and K*b, with dimension MSG_NUM x poly_size
    struct GR128_D4 **z_poly = xcalloc(MSG_NUM, sizeof(void*));
    
    for (size_t i = 0; i < MSG_NUM; ++i) {
        polys[i] = xcalloc(c*poly_size, sizeof(struct GR128_D4));
        poly_buf[i] = xcalloc(c*poly_size, sizeof(struct GR128_D4));
        z_poly[i] = xcalloc(poly_size, sizeof(struct GR128_D4));
    }
    ...
    spdz2k_64_d4_b->keys_b0 = keys_b0;
    spdz2k_64_d4_b->keys_b1 = keys_b1;
    
    spdz2k_64_d4_b->b0 = b0;
    spdz2k_64_d4_b->b1 = b1;
    spdz2k_64_d4_b->bm0 = bm0;
    spdz2k_64_d4_b->bm1 = bm1;

    spdz2k_64_d4_b->polys = polys;
    spdz2k_64_d4_b->poly_buf = poly_buf;
    spdz2k_64_d4_b->z_poly = z_poly;

    spdz2k_64_d4_b->shares = shares;
    spdz2k_64_d4_b->cache = cache;
}
...
void sample_SPDZ2k_64_d4_b_DPF_keys(const struct Param *param, struct KeysHD *keys) {
    ...
    for (size_t i=0; i<c; ++i) {
        for (size_t j=0; j<t; ++j) {
            ...
            struct GR128_D4 beta[MSG_NUM];
            // randomize the value
            RAND_bytes((uint8_t *)beta, sizeof(struct GR128_D4));
            // compute the MAC
            uint128_t K = param->K64;
...
void init_SPDZ2k_64_d4_prod(const struct Param *param, struct SPDZ2k_64_D4_Prod *spdz2k_64_d4_prod) {
    ...
    sample_SPDZ2k_64_d4_prod_DPF_keys(param, keys);
    ...
    for (size_t i = 0; i < m; ++i) {
        // for Tr(z) and Tr(z*K)
        rlt[i] = xcalloc(MSG_NUM*t*t*dpf_block_size, sizeof(uint128_t));
    }
    spdz2k_64_d4_prod->rlt = rlt;

    struct GR128_D4 **z_poly = xcalloc(MSG_NUM, sizeof(void*));
    for (size_t i = 0; i < MSG_NUM; ++i) {
        z_poly[i] = xcalloc(m*t*t*dpf_block_size, sizeof(struct GR128_D4));
    }
    /**
    ...
    struct GR128_D4 **poly_buf = xcalloc(MSG_NUM, sizeof(void*));
    for (size_t i = 0; i < MSG_NUM; ++i) {
        polys[i] = xcalloc(c*c*t*t*m*dpf_block_size, sizeof(struct GR128_D4));
        poly_buf[i] = xcalloc(c*c*t*t*m*dpf_block_size, sizeof(struct GR128_D4));
    }
    ...
    spdz2k_64_d4_prod->keys = keys;
    spdz2k_64_d4_prod->polys = polys;
    spdz2k_64_d4_prod->poly_buf = poly_buf;
    spdz2k_64_d4_prod->z_poly = z_poly;
    spdz2k_64_d4_prod->rlt = rlt;
    spdz2k_64_d4_prod->shares = shares;
    spdz2k_64_d4_prod->cache = cache;
}
...
void run_SPDZ2k_64_d4_b(const struct Param *param, struct SPDZ2k_64_D4_b *spdz2k_64_d4_b, struct GR128_D4 **fft_a, const struct GR128_D4 *zeta_powers) {
    size_t poly_size = param->poly_size;
    struct KeysHD *keys_b0 = spdz2k_64_d4_b->keys_b0;
    struct KeysHD *keys_b1 = spdz2k_64_d4_b->keys_b1;
    uint128_t **b0 = spdz2k_64_d4_b->b0;
    uint128_t **b1 = spdz2k_64_d4_b->b1;
    
    uint128_t **bm0 = spdz2k_64_d4_b->bm0;
    uint128_t **bm1 = spdz2k_64_d4_b->bm1;
    
    struct GR128_D4 **polys = spdz2k_64_d4_b->polys;
    struct GR128_D4 **poly_buf = spdz2k_64_d4_b->poly_buf;
    struct GR128_D4 **z_poly = spdz2k_64_d4_b->z_poly;
    uint128_t *shares = spdz2k_64_d4_b->shares;
    uint128_t *cache = spdz2k_64_d4_b->cache;
     ...
     */
    evaluate_SPDZ2k_64_d4_b_DPF_and_sum(param, keys_b0, fft_a, polys, poly_buf, z_poly, shares, cache, zeta_powers);
    trace_SPDZ2k_64_d4_b_FFT_polys(param, z_poly, b0, bm0, zeta_powers);

    // evaluate for b1 and K*b1
    memset(z_poly[0], 0, poly_size*sizeof(struct GR128_D4));
    memset(z_poly[1], 0, poly_size*sizeof(struct GR128_D4));

    evaluate_SPDZ2k_64_d4_b_DPF_and_sum(param, keys_b1, fft_a, polys, poly_buf, z_poly, shares, cache, zeta_powers);
    trace_SPDZ2k_64_d4_b_FFT_polys(param, z_poly, b1, bm1, zeta_powers);
}
...
void evaluate_SPDZ2k_64_d4_b_DPF_and_sum(
    ...
    evaluate_SPDZ2k_64_d4_b_DPF(param, keys, polys, shares, cache);
    convert_SPDZ2k_64_d4_b_to_FFT(param, polys, zeta_powers);
    multiply_SPDZ2k_64_d4_b_FFT(param, fft_a, polys, poly_buf);
    sum_SPDZ2k_64_d4_b_FFT_polys(param, poly_buf, z_poly);
}

/**
 * Evaluate the DPF keys to polynomials
 * @param polys is of length c*poly_size*2=c*t*block_size*2
 * @param shares should be of length 2*sizeof(struct GR128_D4)/sizeof(uint128_t)*block_size
 * @param cache should be of length 2*sizeof(struct GR128_D4)/sizeof(uint128_t)*block_size
 */
void evaluate_SPDZ2k_64_d4_b_DPF(const struct Param *param, const struct KeysHD *keys, struct GR128_D4 **polys, uint128_t *shares, uint128_t *cache) {
    ...
    for (size_t i = 0; i < c; ++i) {
        for (size_t j = 0; j < t; ++j) {
            ...
            struct GR128_D4 *poly_block0 = &polys[0][poly_index*block_size];
            struct GR128_D4 *poly_block1 = &polys[1][poly_index*block_size];
            ...
            copy_gr128_d4_block(poly_block0, poly_block1, shares, block_size);
            // printf("After copy_gr128_d4_block\n");
        }
    }
}
...
void mult_SPDZ2k_64_d4_list(const struct GR128_D4 *a, const struct GR128_D4 *b, struct GR128_D4 *t, size_t length, const size_t modulus) {
    for (size_t i = 0; i < length; i++) {
        mult_SPDZ2k_64_d4(&a[i], &b[i], &t[i], modulus);
    }
}
...
void sum_SPDZ2k_64_d4_b_FFT_polys(const struct Param *param, struct GR128_D4 **poly_buf, struct GR128_D4 **z_poly) {
    ...
    for (size_t i = 0; i < c; i++) {
        add_SPDZ2k_64_d4_list(&poly_buf[0][i*poly_size], z_poly[0], z_poly[0], poly_size, modulus64);

        add_SPDZ2k_64_d4_list(&poly_buf[1][i*poly_size], z_poly[1], z_poly[1], poly_size, modulus64);
    }
}
...
void add_SPDZ2k_64_d4_list(const struct GR128_D4 *a, const struct GR128_D4 *b, struct GR128_D4 *t, size_t length, const size_t modulus) {
    for (size_t i = 0; i < length; i++) {
        add_SPDZ2k_64_d4(&a[i], &b[i], &t[i], modulus);
    }
}
...
void trace_SPDZ2k_64_d4_b_FFT_polys(const struct Param *param, struct GR128_D4 **src, uint128_t **b, uint128_t **bm, const struct GR128_D4 *powers) {
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < poly_size; ++j) {
            mult_gr128_D4(&powers[i], &src[0][j], &mult_rlt);
            b[i][j] = trace_GR128_d4(&mult_rlt, m);
        }
    }
    // Tr(zeta^j*K*b)
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < poly_size; ++j) {
            mult_gr128_D4(&powers[i], &src[1][j], &mult_rlt);
            bm[i][j] = trace_GR128_d4(&mult_rlt, m);
        }
    }
}
...
/**
 * Parse each block of type uint128_t to GR128_D4. In fact, it converts each uint128_t to GR128_D4 for both the value and MACs.
...
void copy_gr128_d4_block(struct GR128_D4 *poly_block0, struct GR128_D4 *poly_block1, uint128_t *shares, const size_t dpf_block_size) {
    for (size_t w = 0; w < dpf_block_size; ++w) {
        struct GR128_D4 *v = (struct GR128_D4*)&shares[w*DPF_MSG_NUM];
        memcpy(&poly_block0[w], &v[0], sizeof(struct GR128_D4));
        memcpy(&poly_block1[w], &v[1], sizeof(struct GR128_D4));
    }
}
...
void sample_SPDZ2k_64_d4_a_and_tensor(const struct Param *param, struct FFT_GR128_D4_Trace_A *fft_gr128_d4_trace_a) {
    ...
    uint128_t modulus64 = param->modulus64;
    struct GR128_D4 **fft_a = fft_gr128_d4_trace_a->fft_a;
    struct GR128_D4 **fft_a_tensor = fft_gr128_d4_trace_a->fft_a_tensor;
    const size_t poly_size = param->poly_size;
    const size_t c = param->c;
    const size_t m = param->m;

    for (size_t i = 1; i < c; ++i) {
        RAND_bytes((uint8_t *)fft_a[i], sizeof(struct GR128_D4) * poly_size);
    }
    ...
    for (size_t i = 1; i < m; ++i) {
        for (size_t j = 0; j < c; ++j) {
            for (size_t k = 0; k < poly_size; ++k) {
                struct GR128_D4 *prev = &fft_a[(i-1)*c+j][k];
                struct GR128_D4 *cur = &fft_a[i*c+j][k];
                compute_Frob_gr128_d4(prev, cur);
            }
        }
    }
    ...
    for (size_t k = 0; k < m; ++k) {
        for (size_t i = 0; i < c; ++i) {
            for (size_t j = 0; j < c; ++j) {
                mult_gr128_D4_list(fft_a[i], fft_a[k*c+j], fft_a_tensor[(k*c+i)*c+j], poly_size);
            }
        }
    }
}
...
void compute_Frob_gr128_d4(const struct GR128_D4 *prev, struct GR128_D4 *cur) {
    // use primitive polynomial: X^4 + 23254378673668297602512925802836206188*X^3 - 2*X^2 - 23254378673668297602512925802836206189*X + 1
    // X^4: -23254378673668297602512925802836206188*X^3 + 2*X^2 + 23254378673668297602512925802836206189*X - 1
    // X^6: -3*X^3 - 46508757347336595205025851605672412377*X^2 - 23254378673668297602512925802836206186*X + 23254378673668297602512925802836206190

    const static uint128_t a = 23254378673668297602512925802836206188ULL;
    const static uint128_t b = -2ULL;
    const static uint128_t c = -(a+1);

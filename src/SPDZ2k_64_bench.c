...
#define DPF_MSG_NUM sizeof(struct GR128)/sizeof(uint128_t)
...
void SPDZ2k_64_bench_pcg(size_t n, size_t c, size_t t, struct PCG_Time *pcg_time) {
    ...
    size_t s = 57;
    printf("k=%zu, s=%zu\n", k, s);
    init_SPDZ2k_64_bench_params(param, n, c, t, m, k, s);
    ...
    struct FFT_GR128_Trace_A *fft_gr128_trace_a = xcalloc(1, sizeof(struct FFT_GR128_Trace_A));
    init_FFT_GR128_Trace_A(param, fft_gr128_trace_a);
    sample_SPDZ2k_64_a_and_tensor(param, fft_gr128_trace_a);

    struct SPDZ2k_64_b *spdz2k_64_b = xcalloc(1, sizeof(struct SPDZ2k_64_b));
    init_SPDZ2k_64_b(param, spdz2k_64_b);
    // ***************************
    // variables for the product
    struct SPDZ2k_64_Prod *spdz2k_64_prod = xcalloc(1, sizeof(struct SPDZ2k_64_Prod));

    init_SPDZ2k_64_prod(param, spdz2k_64_prod);
    printf("Benchmarking PCG evaluation \n");

    clock_t start_expand_time = clock();
    run_SPDZ2k_64_b(param, spdz2k_64_b, fft_gr128_trace_a->fft_a);
    run_SPDZ2k_64_prod(param, spdz2k_64_prod, fft_gr128_trace_a->fft_a_tensor_maps);
    ...
    free_SPDZ2k_64_prod(param, spdz2k_64_prod);
    free_FFT_GR128_Trace_A(param, fft_gr128_trace_a);
    free_SPDZ2k_64_b(param, spdz2k_64_b);
    free(param);
...
void sample_SPDZ2k_64_a_and_tensor(const struct Param *param, struct FFT_GR128_Trace_A *fft_gr128_trace_a) {
    ...
    uint128_t modulus128 = param->modulus128;
    ...
    for (size_t i = 1; i < c; ++i) {
        for (size_t j = 0; j < poly_size; ++j) {
            fft_a[i][j].c0 = fft_a[i][j].c0 % modulus128;
            fft_a[i][j].c1 = fft_a[i][j].c1 % modulus128;
        }
    }
    for (size_t i = 0; i < c; ++i) {
        for (size_t j = 0; j < poly_size; j++) {
            struct GR128 *cur_a = &fft_a_maps[i][j];
            cur_a->c0 = cur_a->c0 % modulus128;
            cur_a->c1 = cur_a->c1 % modulus128;
        }
    }
    for (size_t i = 0; i < c; ++i) {
        for (size_t j = 0; j < c; ++j) {
            for (size_t k = 0; k < poly_size; ++k) {
                struct GR128 *cur_a = &fft_a_tensor_maps[i*c+j][k];
                cur_a->c0 = cur_a->c0%modulus128;
                cur_a->c1 = cur_a->c1%modulus128;
...
// Multiply two SPDZ2k_64 elements
...
void sample_SPDZ2k_64_b_DPF_keys(const struct Param *param, struct Keys *keys) {
    ...
    for (size_t i=0; i<c; ++i) {
        for (size_t j=0; j<t; ++j) {
            ...
            RAND_bytes((uint8_t *)beta, sizeof(struct GR128));
            beta[0] = beta[0]%modulus128;
            beta[1] = beta[1]%modulus128;
            uint128_t K = param->K128;
            beta[2] = (beta[0]*K)%modulus128;
            beta[3] = (beta[1]*K)%modulus128;
...
void evaluate_SPDZ2k_64_b_DPF(const struct Param *param, const struct Keys *keys, struct GR128 *polys, uint128_t *shares, uint128_t *cache) {
    ...
    for (size_t i = 0; i < c; ++i) {
        for (size_t j = 0; j < t; ++j) {
            ...
            struct GR128 *poly_block = &polys[poly_index*block_size*DPF_MSG_LEN];
            const size_t key_index = poly_index;
            struct DPFKey *dpf_key = keys->dpf_keys_A[key_index];
            DPFFullDomainEval(dpf_key, cache, shares);
            copy_gr128_block(poly_block, shares, DPF_MSG_LEN*block_size);
        }
    }
}
...
void convert_SPDZ2k_64_b_to_FFT(const struct Param *param, struct GR128 *polys) {
    ...
    const size_t n = param->n;
    const size_t poly_size = param->poly_size;
    const uint128_t modulus128 = param->modulus128;
    
    for (size_t i = 0; i < c*DPF_MSG_LEN; ++i) {

        struct GR128 *poly = &polys[i*poly_size];
        fft_recursive_SPDZ2k_64(poly, n, poly_size / 3, modulus128);
    }
}
...
void multiply_SPDZ2k_64_b_FFT(const struct Param *param, struct GR128 **a_polys, const struct GR128 *b_poly, struct GR128 *res_poly) {
    ...
    for (size_t i = 0; i < c; ++i) {
        const struct GR128 *a_poly = a_polys[i];
        for (size_t k = 0; k < DPF_MSG_LEN; ++k) {
            for (size_t j = 0; j < poly_size; ++j) {
                mult_SPDZ2k_64(&a_poly[j], &b_poly[(i*DPF_MSG_LEN+k)*poly_size+j], &res_poly[(i*DPF_MSG_LEN+k)*poly_size+j], modulus128);
            }
        }
    }
}
...
void sum_SPDZ2k_64_b_FFT_polys(const struct Param *param, struct GR128 *poly_buf, struct GR128 *z_poly) {
    ...
    for (size_t i = 0; i < c; ++i) {
        ...
        for (size_t l = 0; l < DPF_MSG_LEN; ++l) {
            for (size_t k = 0; k < poly_size; ++k) {
                struct GR128 *z = &z_poly[l*poly_size+k];
                z->c0 = (z->c0 + poly_buf[poly_index+k].c0) % modulus128;
                z->c1 = (z->c1 + poly_buf[poly_index+k].c1) % modulus128;
            }
        }
    }
}
...
void evaluate_SPDZ_64_b_DPF_and_sum(
    ...
    evaluate_SPDZ2k_64_b_DPF(param, keys, polys, shares, cache);
    convert_SPDZ2k_64_b_to_FFT(param, polys);
    multiply_SPDZ2k_64_b_FFT(param, fft_a, polys, poly_buf);
    sum_SPDZ2k_64_b_FFT_polys(param, poly_buf, z_poly);
}
...
void trace_SPDZ2k_64_b_FFT_polys(const struct Param *param, const struct GR128 *src, uint128_t *b_0, uint128_t *b_1, uint128_t *bm_0, uint128_t *bm_1) {
    const size_t poly_size = param->poly_size;
    const uint128_t modulus128 = param->modulus128;
    const struct GR128 *poly = src;
    // Tr(b) and Tr(zeta*b)
    for (size_t i = 0; i < poly_size; ++i) {
        b_0[i] = (2*poly[i].c0-poly[i].c1)%modulus128;
        b_1[i] = (-(poly[i].c0+poly[i].c1))%modulus128;
    }
    // Tr(K*b) and Tr(zeta*K*b)
    poly = &src[poly_size];
    for (size_t i = 0; i < poly_size; ++i) {
        bm_0[i] = (2*poly[i].c0-poly[i].c1)%modulus128;
        bm_1[i] = (-(poly[i].c0 + poly[i].c1))%modulus128;
    }
}
...
void init_SPDZ2k_64_b(const struct Param *param, struct SPDZ2k_64_b *spdz2k_64_b) {
    ...
    sample_SPDZ2k_64_b_DPF_keys(param, keys_b0);
    struct Keys *keys_b1 = xcalloc(1, sizeof(struct Keys));
    sample_SPDZ2k_64_b_DPF_keys(param, keys_b1);
    // output for (b0,b1) and each has two values for the trace
    uint128_t *b0_0 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *b0_1 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *b1_0 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *b1_1 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *bm0_0 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *bm0_1 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *bm1_0 = xcalloc(poly_size, sizeof(uint128_t));
    uint128_t *bm1_1 = xcalloc(poly_size, sizeof(uint128_t));
    
    struct GR128 *polys = xcalloc(DPF_MSG_LEN*c*poly_size, sizeof(struct GR128));
    struct GR128 *poly_buf = xcalloc(DPF_MSG_LEN*c*poly_size, sizeof(struct GR128));
    struct GR128 *z_poly = xcalloc(DPF_MSG_LEN*poly_size, sizeof(struct GR128));
    uint128_t *shares = xcalloc(DPF_MSG_LEN*DPF_MSG_NUM*block_size, sizeof(uint128_t));
    uint128_t *cache = xcalloc(DPF_MSG_LEN*DPF_MSG_NUM*block_size, sizeof(uint128_t));
    
    spdz2k_64_b->keys_b0 = keys_b0;
    spdz2k_64_b->keys_b1 = keys_b1;
    
    spdz2k_64_b->b0_0 = b0_0;
    spdz2k_64_b->b0_1 = b0_1;
    spdz2k_64_b->b1_0 = b1_0;
    spdz2k_64_b->b1_1 = b1_1;
    spdz2k_64_b->bm0_0 = bm0_0;
    spdz2k_64_b->bm0_1 = bm0_1;
    spdz2k_64_b->bm1_0 = bm1_0;
    spdz2k_64_b->bm1_1 = bm1_1;

    spdz2k_64_b->polys = polys;
    spdz2k_64_b->poly_buf = poly_buf;
    spdz2k_64_b->z_poly = z_poly;
    spdz2k_64_b->shares = shares;
    spdz2k_64_b->cache = cache;
}

void free_SPDZ2k_64_b(const struct Param *param, struct SPDZ2k_64_b *spdz2k_64_b) {
    free_SPDZ2k_64_b_DPF_keys(param, spdz2k_64_b->keys_b0);
    free_SPDZ2k_64_b_DPF_keys(param, spdz2k_64_b->keys_b1);
    free(spdz2k_64_b->b0_0);
    free(spdz2k_64_b->b0_1);
    free(spdz2k_64_b->b1_0);
    free(spdz2k_64_b->b1_1);
    free(spdz2k_64_b->bm0_0);
    free(spdz2k_64_b->bm0_1);
    free(spdz2k_64_b->bm1_0);
    free(spdz2k_64_b->bm1_1);

    free(spdz2k_64_b->polys);
    free(spdz2k_64_b->poly_buf);
    free(spdz2k_64_b->z_poly);
    free(spdz2k_64_b->shares);
    free(spdz2k_64_b->cache);
    free(spdz2k_64_b);
}

void run_SPDZ2k_64_b(const struct Param *param, struct SPDZ2k_64_b *spdz2k_64_b, struct GR128 **fft_a) {
    size_t poly_size = param->poly_size;
    struct Keys *keys_b0 = spdz2k_64_b->keys_b0;
    struct Keys *keys_b1 = spdz2k_64_b->keys_b1;
    uint128_t *b0_0 = spdz2k_64_b->b0_0;
    uint128_t *b0_1 = spdz2k_64_b->b0_1;
    uint128_t *b1_0 = spdz2k_64_b->b1_0;
    uint128_t *b1_1 = spdz2k_64_b->b1_1;

    uint128_t *bm0_0 = spdz2k_64_b->bm0_0;
    uint128_t *bm0_1 = spdz2k_64_b->bm0_1;
    uint128_t *bm1_0 = spdz2k_64_b->bm1_0;
    uint128_t *bm1_1 = spdz2k_64_b->bm1_1;
    
    struct GR128 *polys = spdz2k_64_b->polys;
    struct GR128 *poly_buf = spdz2k_64_b->poly_buf;
    struct GR128 *z_poly = spdz2k_64_b->z_poly;
    uint128_t *shares = spdz2k_64_b->shares;
    uint128_t *cache = spdz2k_64_b->cache;

    // evaluate for b0 and K*b0
    evaluate_SPDZ_64_b_DPF_and_sum(param, keys_b0, fft_a, polys, poly_buf, z_poly, shares, cache);
    trace_SPDZ2k_64_b_FFT_polys(param, z_poly, b0_0, b0_1, bm0_0, bm0_1);
    
    // evaluate for b1 and K*b1
    memset(z_poly, 0, poly_size*sizeof(struct GR128));
    evaluate_SPDZ_64_b_DPF_and_sum(param, keys_b1, fft_a, polys, poly_buf, z_poly, shares, cache);
    trace_SPDZ2k_64_b_FFT_polys(param, z_poly, b1_0, b1_1, bm1_0, bm1_1);
}
...
void init_SPDZ2k_64_prod(const struct Param *param, struct SPDZ2k_64_Prod *spdz2k_64_prod) {
    ...
    sample_SPDZ2k_64_prod_DPF_keys(param, keys);
    // DPF_MSG_LEN indicates the value z and z*K
    // m indicates the number of automorphisms

    // output for Tr(z) and Tr(z*K)
    uint128_t *rlt0 = xcalloc(DPF_MSG_LEN*t*t*dpf_block_size, sizeof(uint128_t));
    // output for Tr(z*zeta) and Tr(z*zeta*K)
    uint128_t *rlt1 = xcalloc(DPF_MSG_LEN*t*t*dpf_block_size, sizeof(uint128_t));
    // The polynomial for z and z*K
    struct GR128 *z_poly0 = xcalloc(m*t*t*dpf_block_size, sizeof(struct GR128));
    // The polynomial for zeta*z and zeta*z*K
    struct GR128 *z_poly1 = xcalloc(m*t*t*dpf_block_size, sizeof(struct GR128));
    /**
     * @param m is for the number of automorphisms
     * @param DPF_MSG_LEN is for the number of outputs
     */
    struct GR128 *polys = xcalloc(DPF_MSG_LEN*c*c*t*t*m*dpf_block_size, sizeof(struct GR128));
    struct GR128 *poly_buf = xcalloc(DPF_MSG_LEN*c*c*t*t*m*dpf_block_size, sizeof(struct GR128));
    uint128_t *shares = xcalloc(DPF_MSG_LEN*DPF_MSG_NUM*dpf_block_size, sizeof(uint128_t));
    uint128_t *cache = xcalloc(DPF_MSG_LEN*DPF_MSG_NUM*dpf_block_size, sizeof(uint128_t));

    spdz2k_64_prod->keys = keys;
    spdz2k_64_prod->polys = polys;
    spdz2k_64_prod->poly_buf = poly_buf;
    spdz2k_64_prod->z_poly0 = z_poly0;
    spdz2k_64_prod->z_poly1 = z_poly1;
    spdz2k_64_prod->rlt0 = rlt0;
    spdz2k_64_prod->rlt1 = rlt1;
    spdz2k_64_prod->shares = shares;
    spdz2k_64_prod->cache = cache;
}


void free_SPDZ2k_64_prod(const struct Param *param, struct SPDZ2k_64_Prod *spdz2k_64_prod) {
    free_SPDZ2k_64_prod_DPF_keys(param, spdz2k_64_prod->keys);
    free(spdz2k_64_prod->polys);
    free(spdz2k_64_prod->poly_buf);
    free(spdz2k_64_prod->z_poly0);
    free(spdz2k_64_prod->z_poly1);
    free(spdz2k_64_prod->rlt0);
    free(spdz2k_64_prod->rlt1);
    free(spdz2k_64_prod->shares);
    free(spdz2k_64_prod->cache);
    free(spdz2k_64_prod);
}
...
void run_SPDZ2k_64_prod(const struct Param *param, struct SPDZ2k_64_Prod *spdz2k_64_prod, struct GR128 **fft_a_tensor_maps) {
    size_t poly_size = param->poly_size;
    struct Keys *keys = spdz2k_64_prod->keys;
    
    uint128_t *rlt0 = spdz2k_64_prod->rlt0;
    uint128_t *rlt1 = spdz2k_64_prod->rlt1;
    
    struct GR128 *polys = spdz2k_64_prod->polys;
    struct GR128 *poly_buf = spdz2k_64_prod->poly_buf;
    struct GR128 *z_poly0 = spdz2k_64_prod->z_poly0;
    struct GR128 *z_poly1 = spdz2k_64_prod->z_poly1;
    uint128_t *shares = spdz2k_64_prod->shares;
    uint128_t *cache = spdz2k_64_prod->cache;

    // evaluate for z and K*z
    evaluate_SPDZ2k_64_prod_DPF_and_sum(param, keys, fft_a_tensor_maps, polys, poly_buf, z_poly0, z_poly1, shares, cache);
    trace_SPDZ2k_64_prod_FFT_polys(param, z_poly0, rlt0);
    trace_SPDZ2k_64_prod_FFT_polys(param, z_poly1, rlt1);
}
...
void evaluate_SPDZ2k_64_prod_DPF_and_sum(
    ...
    evaluate_SPDZ2k_64_prod_DPF(param, keys, polys, shares, cache);
    convert_SPDZ2k_64_prod_to_FFT(param, polys);
    multiply_SPDZ2k_64_prod_FFT(param, fft_a_tensor_maps, polys, poly_buf);
    sum_SPDZ2k_64_prod_FFT_polys(param, poly_buf, z_poly0);
    sum_SPDZ2k_64_prod_FFT_polys_special(param, poly_buf, z_poly1);
}
            ...
            for (size_t w = 0; w < m; ++w) {
                for (size_t k = 0; k < t; k++) {
                    for (size_t l = 0; l < t; l++) {
                        ...
                        struct DPFKey *dpf_key = keys->dpf_keys_A[key_index];
                        struct GR128 *poly_block = &polys[key_index*DPF_MSG_LEN*dpf_block_size];
                        DPFFullDomainEval(dpf_key, cache, shares);
                        copy_gr128_block(poly_block, shares, DPF_MSG_LEN*dpf_block_size);
                    }
                }
            }
        }
...
void multiply_SPDZ2k_64_prod_FFT(const struct Param *param, struct GR128 **a_polys, const struct GR128 *b_poly, struct GR128 *res_poly) {
    ...
    for (size_t i = 0; i < c*c; ++i) {
        ...
        mult_SPDZ2k_64(a_polys[i], &b_poly[poly_index], &res_poly[poly_index], modulus128);
        mult_SPDZ2k_64(a_polys[i], &b_poly[poly_index+poly_size], &res_poly[poly_index+poly_size], modulus128);
    }
    ...
    for (size_t i = 0; i < c*c; ++i) {
        ...
        for (size_t k = 0; k < DPF_MSG_LEN; ++k) {
            ...
            for (size_t j = 0; j < poly_size*m; ++j) {
               mult_SPDZ2k_64(&a_poly[j], &b_poly[poly_index+j], &res_poly[poly_index+j], modulus128);
            }
        }
    }
}
    ...
    for (size_t i = 0; i < c*c; ++i) {
        for (size_t l = 0; l < DPF_MSG_LEN; ++l) {
            for (size_t j = 0; j < poly_size*m; ++j) {
                ...
                z_poly[j].c0 = (z_poly[j].c0+poly_buf[index].c0)%modulus128;
                z_poly[j].c1 = (z_poly[j].c1+poly_buf[index].c1)%modulus128;
            }
        }
    }
}
    ...
    for (size_t i = 0; i < c*c; ++i) {
        for (size_t l = 0; l < DPF_MSG_LEN; ++l) {
            for (size_t w = 0; w < m; ++w) {
                ...
                if (w == 0) {
                    for (size_t j = 0; j < poly_size; ++j) {
                        z_poly[w*poly_size+j].c0 = (z_poly[w*poly_size+j].c0+poly_buf[poly_index+j].c1-poly_buf[poly_index+j].c0)%modulus128;
                        z_poly[w*poly_size+j].c1 = (z_poly[w*poly_size+j].c1-poly_buf[poly_index+j].c0)%modulus128;
                    }
                } else {
                    for (size_t j = 0; j < poly_size; ++j) {
                        z_poly[w*poly_size+j].c0 = (z_poly[w*poly_size+j].c0+poly_buf[poly_index+j].c0)%modulus128;
                        z_poly[w*poly_size+j].c1 = (z_poly[w*poly_size+j].c1+poly_buf[poly_index+j].c1)%modulus128;
                    }
                }
            }
        }
...
void trace_SPDZ2k_64_prod_FFT_polys(const struct Param *param, struct GR128 *z_poly, uint128_t *rlt) {
    ...
    for (size_t i = 0; i < poly_size*DPF_MSG_LEN; ++i) {
        rlt[i] = (2*z_poly[i].c0-z_poly[i].c1)%modulus128;
    }
}

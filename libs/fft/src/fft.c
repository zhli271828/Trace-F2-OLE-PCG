...
void fft_recursive_mal128_f16_trace(
    uint128_t *coeffs,
    const uint8_t *zeta_powers,
    const size_t num_vars,
    const size_t num_coeffs,
    ...
    if (num_vars > 1) {
        for (size_t i = 0; i < base; ++i) {
            // apply FFT on each branch
            fft_recursive_mal128_f16_trace(&coeffs[i*num_coeffs], zeta_powers, num_vars-1, num_coeffs/base, param, base);
        }
    }
    // temp variables to store intermediate values
    uint128_t mult;
    ...
    for (size_t j = 0; j < num_coeffs; j++) {
        ...
        for (size_t k = 0; k < base; ++k) {
            for (size_t i = 0; i < base; ++i) {
                 ...
                 * TODO: zeta_powers should be GF(2^128) elements.
                 */
                mult = mult_mal128_f16(coeffs_pos[i][j], zeta_powers[k*i%base]);
                t_coeffs[k] ^= mult;
            }
        }
        // copy temp to the value back
